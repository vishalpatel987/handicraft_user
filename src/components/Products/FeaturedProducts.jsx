    ...
    const fetchProducts = async () => {
      try {
        ...
        try {
          ...
          productsCache = productsWithCategoryNames;
          cacheTimestamp = Date.now();
          setProducts(productsWithCategoryNames);
        } catch (error) {
          console.error('Error fetching categories for featured products:', error);
      ...
      <div className="container mx-auto px-2 sm:px-4 lg:px-6">
        ...
        >
            {displayedProducts.map((product, index) => (
              <motion.div key={product._id || product.id || `featured-${index}`} variants={itemVariants}>
